...
namespace PMSClient.ViewModel
{
    public class MaterialInventoryOutEditVM : BaseViewModelEdit
    {
        ...
        public void SetBySelect(DcMaterialInventoryIn model)
        {
            try
            {
                using (var service = new MaterialInventoryServiceClient())
                {
                    ...
                    if(PMSDialogService.ShowYesNo("请问", $"要将对应[{model.PMINumber} {model.Composition} {model.MaterialLot}]\n的入库记录标记为出库吗？"))
                    {
        ...
        private void ActionSave()
        {
            ...
            //检查PMINumber是否符合格式规范
            VMHelper.CommonVMHelper.CheckPMINumber(CurrentMaterialInventoryOut.PMINumber);
            try
            {
