...
namespace PMSClient.Components.ImageGallery
{
    ...
    /// 将超声照片汇集成图集
    /// </summary>
    public class GalleryServiceTest440
    {
        public GalleryServiceTest440()
        {
        ...
        {
            var targets = tests.Where(i => i.ProductID.Contains("#"))
                                       .OrderByDescending(i => i.ProductID).ToList();

            #region 创建word文档
            string filename = $"Target(only 440 series) Gallery from {year_start}-{month_start}to {year_end}-{month_end}.docx";
